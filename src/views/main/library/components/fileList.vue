    ...
    <el-container style="position: relative">
        <el-header>
            <el-row class="row-bg" :gutter="20">               
                <el-col :span="10">
                    <div class="flexBox">
                        <el-button size="small" plain @click="createDiretory">新建文件夹</el-button>
                        <!-- 上传 -->
                        <el-button size="small" plain @click="upLoadFile">上传<i class="el-icon-upload el-icon--right"></i></el-button>
                        <!-- 下载 -->
                        <el-button size="small" plain @click="downLoadFile" :disabled="show">下载<i class="el-icon-download el-icon--right"></i></el-button>
                        <!-- 文件操作 -->
                        <el-dropdown trigger="click">
                                ...
                                <el-dropdown-menu slot="dropdown">
                                    ...
                                    <el-dropdown-item class="moveFile_btn_dad">
                                        <el-button size="mini" class="moveFile_btn" type="text" @click="openDialog_move('copy')">复制</el-button>
                                        <el-dialog title="复制到" :visible.sync="copeDialog" append-to-body class="fixedWH" width="550px">
                                            <el-input placeholder="当前路径" v-model="selecPath" :disabled="true">
                                            </el-input>
                                            <div class="border">
                                                <el-tree class="filter-tree" :props="props" :filter-nodeDate-method="filterNode" highlight-current lazy @node-click="handleNodeClick" :load="lazyLoadTreeDir" ref="tree">
                                                </el-tree>
                                            </div>
                                            <div slot="footer" class="dialog-footer">
                                                <el-button @click="copeDialog = false">取 消</el-button>
                                                <el-button type="primary" @click="moveOk('copy')" plain>确 定</el-button>
                                            </div>
                                        </el-dialog>
                                    </el-dropdown-item>
                                    <!-- 移动 -->
                                    <el-dropdown-item class="moveFile_btn_dad">
                                        <el-button size="mini" class="moveFile_btn" type="text" @click="openDialog_move('move')">移动</el-button>
                                        <el-dialog title="移动到" :visible.sync="moveDialog" append-to-body class="fixedWH" width="550px">
                                            <el-input placeholder="当前路径" v-model="selecPath" :disabled="true">
                                            </el-input>
                                            <div class="border">
                                                <el-tree class="filter-tree" :props="props" :filter-nodeDate-method="filterNode" highlight-current lazy @node-click="handleNodeClick" :load="lazyLoadTreeDir" ref="tree">
                                            ...
                                            <div slot="footer" class="dialog-footer">
                                                ...
                                                <el-button type="primary" @click="moveOk('move')" plain>确 定</el-button>
                                            </div>
                                        </el-dialog>
                                    </el-dropdown-item>
                                </el-dropdown-menu>
                        ...
                        <!-- 删除 -->
                        <el-button  size="small" plain @click="deleteDialogVisible = true" :disabled="show">删除</el-button>
                    </div>
                </el-col>
                <!-- 搜索框 -->
                <el-col :span="6" :offset="3">
                    <el-input placeholder="请输入" size="small" v-model="searchFile" @change="searchClick">
                        <i slot="prefix" class="el-input__icon el-icon-search cursor" @click="searchClick"></i>
                    </el-input>
                </el-col>
                <!-- grid布局切换按钮组 -->
                <el-col :span="5">
                    <div class="switch">
                        <el-button-group>
                            <el-button size="small" plain icon="el-icon-s-unfold" @click="setTable"></el-button>
                            <el-button size="small" plain @click="setGrid" icon="el-icon-s-grid"></el-button>
                        </el-button-group>
                    </div>
                </el-col>
            </el-row>
            ...
            <el-col :span="24">
                <div class="bread" style="margin: 12px">
                    <el-breadcrumb separator-class="el-icon-arrow-right">
                        <el-breadcrumb-item v-for="(item, index) in pathbread" :key="index" :to="{
                path: item.path,
        ...
        <el-main >    
            <!-- 列表 -->
            <el-table  v-if="this.layout == 'table'" :data="tableData" class="tableStyle" row-key="id" @select="selec" :default-expand-all="false" @row-dblclick="switchDir" @row-contextmenu="rightMenuMethod" :highlight-current="true">
                <el-table-column fixed="left" type="selection" width="50"> </el-table-column>
                <el-table-column  label="名字" sortable>
                    <template  slot-scope="scope">
                      <div class="cellLater">
                        <v-icon :class="scope.row.isdir == 1 ? 'dir dirFile' : 'file dirFile'" :name="scope.row.isdir ? 'folder' : 'file'" scale="2" />
                            <el-tooltip :content="scope.row.server_filename" effect="light" placement="right-start" :offset="800" :open-delay = '900'>
                            <span class="listText">{{
            ...
            <div class="gridContainer" v-if="layout == 'grid'">
                <div  v-for="(item, index) of tableData" :key="index" @click.capture="changeIn(item)" >
                    <el-tooltip :content="item.server_filename" effect="light" placement="bottom" :open-delay = '900'>

                      <div>
                        <v-icon :class="item.isdir? 'dir' : 'file'" :name="item.isdir ? 'folder' : 'file'" scale="2" />
            ...
            <!-- 创建文件夹模态框 -->
            <el-dialog title="新建" :visible.sync="centerDialogVisible" width="437px" class="createClass">
                <div class="form">
                    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px">
                        ...
                        <el-form-item class="createFlex">
                            ...
                            <el-button plain type="primary"  @click="submitForm()">立即创建</el-button>
                        </el-form-item>
                    </el-form>
                </div>
            </el-dialog>
            <!-- 重命名模态框 -->
            <el-dialog title="重命名提示" :visible.sync="centerDialogVisible2" width="437px">
                ...
                <span slot="footer" class="dialog-footer">
                    ...
                    <el-button type="primary" plain @click="changeDirName">确 定</el-button>
                </span>
            </el-dialog>
            <!-- 删除模态框 -->
             <el-dialog title="删除提示" :visible.sync="deleteDialogVisible" width="437px">
                <i class="el-icon-warning" style="font-size: 1.5em;margin-right: 5px;color:rgb(64,158,255);vertical-align: middle;"></i>
                <span style="margin:0 auto">确认删除？</span>
                <span slot="footer" class="dialog-footer">
                    <el-button @click="deleteDialogVisible = false">取 消</el-button>
                    <el-button type="primary" plain @click="deleteFile(rightMenuIndex,rightMenuRow)">确 定</el-button>
                </span>
            </el-dialog>
            <!-- 右键菜单 -->
              ...
              <div ref="rightMenu" v-show="rightMenushow" class="rightMenu">
                <ul>
                    <el-button size="mini" type="text"  class="renameBtnColor" @click="switchDir(rightMenuRow)">打开</el-button>
                </ul>
                <ul>
                    <el-button size="mini" type="text"  class="renameBtnColor" @click="openDialog_move('copy')">复制</el-button>
                    <el-button size="mini" type="text"  class="renameBtnColor" @click="openDialog_move('move')">移动</el-button>
                </ul>
                <ul>
                    <el-button  size="mini" slot="reference" type="text" class="renameBtnColor" @click="deleteDialogVisible = true">
                        删除
                    </el-button>                                
                    <el-button size="mini" type="text"  class="renameBtnColor" @click="editFileName(rightMenuIndex,rightMenuRow)">重命名
                    </el-button>
                </ul>
             </div>
              </transition>
...
.rightMenu>ul,.rightMenu>ul>button{
    list-style: none;
    cursor: default;
